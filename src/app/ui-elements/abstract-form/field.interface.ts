...
interface SelectConfig {
  ...
  // An event emiiter that is used to send notice of a selection event back to other fields in the group
  ...
  selectionOptions?: (queryParam: string) => Observable<SelectOptions[]>;
}
...
interface UiConfig {
  ...
  /*The fraction that the field should take up based on the number of grid columns. If set to 0.5 and there are 2 columns,
  this will span half the width of the form.
...
export interface FieldConfig extends SelectConfig, InputConfig, TextareaConfig, UiConfig {
  ...
  parentFieldName?: string; // If fields are in sequential order, this is used to identify the parent field. This would be the 'name'
  // field of the parent field
  returnName?: string; // The default field name in the returned object of the form contents uses the 'name' property.
  // This overrides only that
  ...
  disabled?: boolean | Observable<boolean>; // If the field is disabled or not. Can take a boolean or observable
  // if more complicated behavior is needed
